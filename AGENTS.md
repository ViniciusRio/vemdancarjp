...
- **Seja específico.** Nomeie arquivos, funções e símbolos (`src/stores/agendaStore.ts:fetchEvents`) em vez de perguntar sobre "a app".
- **Plane antes de construir.** Para tarefas grandes, peça um plano primeiro (Plan mode / Kimi K3), revise e depois implemente. Evita retrabalho por chute.
- **Sem explicações genéricas.** Não pergunte "explique como o Vue funciona". Pergunte sobre arquivos reais neste repo.
- **Prefira ferramentas de leitura.** Use `read`/`glob`/`grep` em vez de comandos de shell como `cat`/`find`/`grep` no Bash. Não rode `ls` quando `glob` resolve.
- **Reutilize AGENTS.md e skills.** Este arquivo e `skills/*.md` já codificam as convenções — referencie-os em vez de colar contexto em todo prompt.
- **Escope o diff.** Peça a menor mudança que satisfaça o objetivo; rejeite refactors não relacionados embutidos.
- **Verifique uma vez.** Rode a sequência de verificação (`npm run lint && npm run type-check && npm run test:unit`) uma vez ao final, não após cada edição.
- **Escale deliberadamente.** Só troque para Kimi K3 quando a profundidade de raciocínio exigir; o custo de token é maior.
